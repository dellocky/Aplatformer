...
func _physics_process(delta):
	if abs(position.x - player.position.x) < AGGRO_DISTANCE:
		tic(delta)
		get_direction()
...
func get_direction(): #attempts to kite the player by checking wether the player is in attack range or too close
...
func attack():
	...
	arrow.momentum = velocity.x
	if direction_face == "right":
		arrow.position = $"Right Bow".global_position
		arrow.direction = 1
	elif direction_face == "left":
		arrow.position = $"Left Bow".global_position
		arrow.direction = -1
